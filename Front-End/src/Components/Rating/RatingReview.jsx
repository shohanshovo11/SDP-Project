...
function RatingReview({email}) {
  ...
  async function getratedUser() {
    const response = await axios.get(`http://localhost:5000/studentshow/${email}`);
 ...
 async function giveRating(e) {
  ...
  const reqbody={
    ...
    const response = await axios.post(`http://localhost:5000/giverate/${email}`,reqbody);
    const data = response.data;
  ...
  return (
    <div className='ama'>
        ...
        <img src={ratedUser.profileImgUrl} className='Pict'/>
          <div className='kathala'>{ratedUser.name}
          </div>
          <div className='peyaraa'>{ratedUser.email}
          </div>
        </div>


        <form className='patal' onSubmit={giveRating}>
         <div className='akash'>Give A Rating
          </div>
          <div className='mati'>
         ...
         <div className='lebu'>
         <div className='korola'>Give A Review
          </div>
          <textarea className='jambura' onChange={(e) => {
